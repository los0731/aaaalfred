...
def fnSnapshot(stock):
	...
	with req.Session() as s:
		...
		get_url = s.get('https://comp.fnguide.com/SVO2/ASP/SVD_Main.asp?pGB=1&gicode=A{}'.format(str(stock)), headers=headers).text
...
def fnConsensus(stock):
	...
	with req.Session() as s:
		...
		r = req.get(url, headers=headers).json()['comp']
